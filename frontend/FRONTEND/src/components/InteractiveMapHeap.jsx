...
import { BASE_URL_DEV } from '../environment/index';
...
const InteractiveMap = () => {
		...
		const fetchUserData = async () => {
			try {
				...
				const filteredUsers = usuarios.filter((usuario) => {
					const isEligible =
						usuario.Rol === 'comprador' || usuario.Rol === 'vendedor';
					if (isEligible) {
						console.log('Usuario que pasa el filtro:', usuario);
					}
					return isEligible;
				});

				const transformedPlaces = filteredUsers.map((usuario) => ({
					...
					imageUrl:
						usuario.Foto ||
						'https://cdn-icons-png.flaticon.com/512/219/219983.png', // Imagen por defecto
				}));

				console.log('Usuarios transformados:', transformedPlaces);
	...
	useEffect(() => {
		...
		places.forEach((location) => {
			const { latitude, longitude, name, rol, imageUrl } = location;
      ...
      `);
			...
			marker
				.getElement()
				.addEventListener('mouseenter', () => popup.addTo(mapRef.current));
			marker.getElement().addEventListener('mouseleave', () => popup.remove());

			marker.getElement().addEventListener('click', () => {
				window.location.href = `http://${BASE_URL_DEV}:5173/profile/${rol}/${id}`;
				console.log(
					'Redirigiendo a:',
					`http://${BASE_URL_DEV}:5173/profile/${rol}/${id}`
				);
			});
		});
